...
public class HotSearch {
    ...
    public static void sort_List() {
        ...
        for(HotSearchContent hs:boughtList) {
            if(hs.paid_index>hotList_tmp.size()){ // 购买位置超过索引范围
                ...
                hotList_tmp.add(hotList_tmp.size(),hs);
            }
            else
    ...
    public void showHotSearch() {
        if(hotSearchList.isEmpty()) {
            System.out.println("Warning：暂无热搜榜单！无法查看！\n");
        }
        else {
            /*
    ...
    public void addHotSearch(String searchName1) {
        ...
        for(HotSearchContent hs1:hotSearchList) {
            if(searchName1.equals(hs1.item_name)) {
                System.out.println("Warning：该热搜已存在！\n");
                flg=false;
                break;
            }
        }
        if(flg) { // 新热搜
            ...
            sort_List(); // 排序
            System.out.println("Tips：当前热搜添加成功！\n");
        }
    }
    ...
    public void addHotSearch_Super(String searchName2) {
        ...
        for(HotSearchContent hs1:hotSearchList) {
            if(searchName2.equals(hs1.item_name)) {
                System.out.println("Warning：该超级热搜已存在！\n");
                flg=false;
                break;
            }
        }
        if(flg) { // 新超级热搜
            ...
            hotSearchList.add(hsc);
            sort_List(); // 排序
            System.out.println("Tips：当前超级热搜添加成功！\n");
        }
    }
            ...
            for(HotSearchContent hs1:hotSearchList) {
                if(voteName.equals(hs1.item_name)) {
                    ...
                    if(hs1.isSuperClass) { // 超级热搜，票数多投一倍
                        hs1.vote_num+=voteNum;
                        System.out.println("Tips：已给超级热搜《"+voteName+"》投票成功！该超级热搜当前"+hs1.vote_num+"票\n");
                    }
                    else
                        System.out.println("Tips：已给《"+voteName+"》投票成功！该热搜当前"+hs1.vote_num+"票\n");
                    sort_List(); // 排序
                    flg=false;
                }
            }
            if(flg) { // 被投票热搜不存在
                System.out.println("Warning：当前被投票热搜不存在！投票失败！\n");
            }
        }
    }
    ...
    public void buy_Search(String name_buy,int pos,int money) {
        if(hotSearchList.isEmpty()) {
            System.out.println("Warning：暂无热搜榜单！无法购买热搜！\n");
        }
        else if(money<1) {
            System.out.println("Warning：购买金额无效！需至少1元起购！\n");
        }
        else {
            ...
            if(pos>=(hotSearchList.size())) { // 购买位置大于热搜数目，直接放在最后
                ...
                if(hsc_tmp.isBought) {
                    boughtList.remove(hsc_tmp); // 如原先已被购买，则在购买列表里删除原先的对象，用新的替代
                }
                hsc_tmp.isBought=true; // 购买成功
                hsc_tmp.paid=money;
                //hsc_tmp.paid_index=hotSearchList.size();
                hsc_tmp.paid_index=pos-1;
                hotSearchList.add(hotSearchList.size(),hsc_tmp);
                boughtList.add(hsc_tmp); // 添加入购买集合
                sort_boughtList();
                System.out.println("Tips：你的热搜已购买成功！《"+hsc_tmp.item_name+"》已位于热搜榜最后一位！\n");
            }
            else {
                ...
                if(hsc2.isBought) { // 该位置原热搜也被购买
                    if(money<=hsc2.paid) {
                        System.out.println("Warning：购买金额不足！购买失败！\n");
                        can_buy_flg=false;
                    }
                }
                if(can_buy_flg) {
                    ...
                    System.out.println("Tips：你的热搜已购买成功！《"+hsc_tmp.item_name+"》已位于热搜第"+pos+"位！\n");
                }
            }
        }
    }
